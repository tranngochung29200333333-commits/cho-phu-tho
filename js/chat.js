...
(function(){
  ...
  async function getProfile(id){
    ...
    const r=await supabaseClient.from('profiles').select('id,full_name,phone,shop_name,shop_description,avatar_url,verified_at').eq('id',id).maybeSingle();
    const p=r.data||null;if(p)profileCache.set(id,p);return p;
  }
  ...
  async function init(){
    ...
    if(activeUser)await loadThread(activeUser,activeListing);
    subscribeRealtime();
    pollTimer=setInterval(()=>{if(activeUser&&!loadingThread)loadThread(activeUser,activeListing);},30000);
  }
  function renderShell(){
    document.getElementById('chatShell').innerHTML='<aside class="chat-list"><div class="chat-list-head"><div><strong>Trao đổi online</strong><small class="chat-online-status">● Đang kết nối</small></div><button id="chatRefresh" type="button" title="Làm mới">↻</button></div><div id="conversationList"><div class="loading">Đang tải...</div></div></aside><section class="chat-main"><div id="chatHeader" class="chat-header"><div><strong>Chọn cuộc trò chuyện</strong><small>Trao đổi trực tiếp</small></div></div><div id="messageList" class="message-list"><div class="empty-state"><p>Chọn nhà bán hàng để bắt đầu trao đổi.</p></div></div><form id="messageForm" class="message-form"><input id="messageInput" maxlength="1000" placeholder="Nhập tin nhắn..." autocomplete="off"><button type="submit">Gửi</button></form></section>';
    document.getElementById('chatRefresh').onclick=loadConversations;
  ...
  async function loadConversations(){
    ...
    if(error){box.innerHTML='<div class="empty-state"><p>Không tải được hội thoại.</p></div>';return;}
    ...
    const ids=[...new Set([...map.values()].map(m=>m.sender_id===me.id?m.receiver_id:m.sender_id))];
    await Promise.all(ids.map(id=>getProfile(id)));
    ...
    box.querySelectorAll('.conversation-item').forEach(b=>b.onclick=()=>{activeUser=b.dataset.user;activeListing=b.dataset.listing||null;loadConversations();loadThread(activeUser,activeListing);});
  }
  async function loadThread(userId,lid){
    ...
    try{
      ...
      if(error)return;
      const rows=(data||[]).filter(m=>!lid||String(m.listing_id||'')===String(lid));
      const prof=await getProfile(userId),box=document.getElementById('messageList'),header=document.getElementById('chatHeader');if(!box||!header)return;
      ...
      box.innerHTML=rows.length?rows.map(m=>`<div class="message-row ${m.sender_id===me.id?'mine':''}"><div class="message-bubble">${esc(m.body).replace(/\n/g,'<br>')}<small>${time(m.created_at)}</small></div></div>`).join(''):'<div class="empty-state"><p>Hãy gửi lời chào đầu tiên 👋</p></div>';
  ...
  function subscribeRealtime(){
    ...
    realtimeChannel=supabaseClient.channel('messages-realtime-'+me.id)
      .on('postgres_changes',{event:'INSERT',schema:'public',table:'messages'},payload=>{
        ...
        const related=m.sender_id===me.id||m.receiver_id===me.id;if(!related)return;
        if(m.sender_id===activeUser||m.receiver_id===activeUser){
          const sameListing=String(m.listing_id||'')===String(activeListing||'');
          if(sameListing||!activeListing){loadThread(activeUser,m.listing_id||null);}
        }
        loadConversations();
        if(m.receiver_id===me.id&&m.sender_id!==activeUser){
          const namePromise=getProfile(m.sender_id);namePromise.then(p=>{if(document.hidden&&'Notification'in window&&Notification.permission==='granted')new Notification('Tin nhắn mới',{body:personName(p)+': '+String(m.body||'').slice(0,80)});});
        }
      }).subscribe(status=>{
        const el=document.querySelector('.chat-online-status');if(el)el.textContent=status==='SUBSCRIBED'?'● Đang kết nối':'○ Đang kết nối lại...';
      });
    if('Notification'in window&&Notification.permission==='default')Notification.requestPermission().catch(()=>{});
  }
  async function sendMessage(e){
    e.preventDefault();if(!activeUser){alert('Hãy chọn nhà bán hàng hoặc cuộc trò chuyện.');return;}
    const input=document.getElementById('messageInput'),body=input.value.trim();if(!body)return;
    const {error}=await supabaseClient.from('messages').insert({sender_id:me.id,receiver_id:activeUser,listing_id:activeListing?Number(activeListing):null,body});
    if(error){alert('Không thể gửi tin nhắn: '+error.message);return;}
