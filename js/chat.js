...
(function(){
  ...
  async function loadThread(userId,lid){
    ...
    try{
      ...
      const rows=data||[];const prof=await getProfile(userId),box=document.getElementById('messageList'),header=document.getElementById('chatHeader');if(!box||!header)return;
  ...
  async function sendMessage(e){
    ...
    const numericListing=activeListing&&/^\d+$/.test(String(activeListing))?Number(activeListing):null;
    const {data,error}=await supabaseClient.from('messages').insert({sender_id:me.id,receiver_id:activeUser,listing_id:numericListing,body}).select('id,listing_id,sender_id,receiver_id,body,created_at,read_at').single();
