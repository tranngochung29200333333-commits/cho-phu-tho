(function(){
  ...
  async function load(){
    ...
    if(!user){location.href='dang-nhap.html?next='+encodeURIComponent(location.href);return;}
  ...
  async function save(){
    ...
    const button=document.getElementById('saveEditButton');button.disabled=true;button.textContent='Đang lưu...';
    try{
      const title=document.getElementById('editTitle').value.trim(),category=document.getElementById('editNetwork').value,subcategory=document.getElementById('editSubcategory').value,location=document.getElementById('editLocation').value,description=document.getElementById('editDescription').value.trim(),price=Number(document.getElementById('editPrice').value||0),phone=document.getElementById('editContactPhone').value.trim(),zalo=document.getElementById('editContactZalo').value.trim(),sim=document.getElementById('editSimNumber').value.trim();
      if(!title)return alert('Vui lòng nhập tiêu đề tin.');
      if(!category)return alert('Vui lòng chọn danh mục.');
      if(!location)return alert('Vui lòng chọn khu vực.');
      const {error}=await supabaseClient.from('listings').update({sim_number:sim,title,price,network:category,category,subcategory,location,description,contact_phone:phone,contact_zalo:zalo,status:'pending'}).eq('id',id).eq('seller_id',user.id);
      if(error)throw error;
      alert('Đã lưu thay đổi. Tin được chuyển về chờ Admin duyệt lại.');
      location.href='quan-ly-tin.html';
