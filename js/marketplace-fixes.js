...
(function(){
  ...
  function installDesktopMessageLink(){
    const topbar=document.querySelector('.topbar');
    const account=document.getElementById('accountArea');
    if(!topbar||!account||document.getElementById('desktopMessageLink'))return;
    const link=document.createElement('a');
    link.id='desktopMessageLink';
    link.className='desktop-message-link';
    link.href='chat.html';
    link.innerHTML='💬 <span>Tin nhắn</span>';
    link.title='Mở tin nhắn';
    topbar.insertBefore(link,account);
    if(!document.getElementById('desktop-message-link-style')){
      const s=document.createElement('style');s.id='desktop-message-link-style';
      s.textContent='.desktop-message-link{display:inline-flex;align-items:center;gap:6px;height:40px;padding:0 14px;border:1px solid #e5e5e5;border-radius:12px;background:#fff;color:#222;text-decoration:none;font-weight:800;white-space:nowrap;transition:.18s ease}.desktop-message-link:hover{border-color:#ffbf00;background:#fff9df;transform:translateY(-1px)}@media(max-width:760px){.desktop-message-link{display:none}}';
  ...
  function loadHomepageRedesign(){
    if(!document.body.classList.contains('home-body')||document.getElementById('home-redesign-css'))return;
    const link=document.createElement('link');
    link.id='home-redesign-css';
    link.rel='stylesheet';
    link.href='css/home-redesign.css?v=20260917-1';
