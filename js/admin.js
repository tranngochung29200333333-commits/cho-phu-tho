...
async function approveSeller(applicationId,userId){
 ...
 const p=await supabaseClient.from("profiles").update({role:"seller",verified_at:new Date().toISOString()}).eq("id",userId);
 if(p.error)return alert(p.error.message);
 await supabaseClient.from("notifications").insert({user_id:userId,type:'seller_approved',title:'Tài khoản nhà bán hàng đã được duyệt',body:'Bạn đã có thể đăng tin trên Chợ Phú Thọ.',url:'dang-tin.html'});
...
async function setListingStatus(id,status){const {data,error}=await supabaseClient.from("listings").update({status}).eq("id",id).select("seller_id,title").single();if(error)return alert(error.message);if(status==='approved'&&data?.seller_id)await supabaseClient.from('notifications').insert({user_id:data.seller_id,type:'listing_approved',title:'Tin đăng đã được duyệt',body:`Tin “${data.title}” đã được hiển thị công khai.`,url:`chi-tiet.html?id=${id}`});loadListings();loadStats();}
...
async function loadUsers(){const box=document.getElementById("adminUsers");const {data,error}=await supabaseClient.from("profiles").select("id,full_name,phone,role,shop_name,verified_at,created_at").order("created_at",{ascending:false}).limit(200);if(error){box.innerHTML=adminHtml(error.message);return;}box.innerHTML=(data||[]).map(u=>`<div class="user-row"><div><b>${adminHtml(u.full_name||"Chưa đặt tên")}</b><span>${adminHtml(u.phone||"—")}${u.shop_name?` · 🏪 ${adminHtml(u.shop_name)}`:''}</span></div><div><span class="listing-status">${adminHtml(u.role)}</span>${u.verified_at?'<small>Đã xác minh</small>':''}<small>${new Date(u.created_at).toLocaleDateString('vi-VN')}</small></div></div>`).join("")||'<div class="empty-state">Chưa có người dùng.</div>';}
...
async function loadReviews(){const box=document.getElementById('adminReviews');const {data,error}=await supabaseClient.from('reviews').select('id,listing_id,seller_id,reviewer_id,rating,body,status,created_at').order('created_at',{ascending:false}).limit(100);if(error){box.innerHTML=adminHtml(error.message);return;}box.innerHTML=(data||[]).map(r=>`<article class="admin-listing"><span class="listing-status">${adminHtml(r.status)}</span><p><b>${'★'.repeat(Number(r.rating))}</b> · Tin #${adminHtml(r.listing_id||'—')}</p><p>${adminHtml(r.body||'Không có nhận xét.')}</p><p class="muted">${new Date(r.created_at).toLocaleString('vi-VN')}</p><div class="admin-actions">${r.status!=='approved'?`<button onclick="setReviewStatus('${r.id}','approved')">✅ Duyệt</button>`:''}${r.status!=='hidden'?`<button onclick="setReviewStatus('${r.id}','hidden')">🙈 Ẩn</button>`:''}<button onclick="deleteReview('${r.id}')">🗑 Xóa</button></div></article>`).join('')||'<div class="empty-state">Chưa có đánh giá.</div>';}
async function setReviewStatus(id,status){const {error}=await supabaseClient.from('reviews').update({status}).eq('id',id);if(error)return alert(error.message);loadReviews();}
async function deleteReview(id){if(!confirm('Xóa đánh giá này?'))return;const {error}=await supabaseClient.from('reviews').delete().eq('id',id);if(error)return alert(error.message);loadReviews();}
async function bootAdmin(){if(!(await requireAdmin()))return;await Promise.all([loadStats(),loadApplications(),loadListings(),loadUsers(),loadReports(),loadReviews()]);}
window.viewKyc=viewKyc;window.approveSeller=approveSeller;window.rejectSeller=rejectSeller;window.setListingStatus=setListingStatus;window.deleteAdminListing=deleteAdminListing;window.resolveReport=resolveReport;window.loadListings=loadListings;window.setReviewStatus=setReviewStatus;window.deleteReview=deleteReview;window.logoutUser=async()=>{await supabaseClient.auth.signOut();location.href='index.html';};
